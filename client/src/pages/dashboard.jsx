...
const Dashboard = () => {
  ...
  const getTaskData = async () => {
    try { 
      ...
      // Calculate counts for different task stages
      const counts = data.reduce((acc, task) => {
        acc.total = data.length;
        if (task.stage) {
          acc[task.stage.toLowerCase()] = (acc[task.stage.toLowerCase()] || 0) + 1;
        }
        return acc;
      }, {
        total: 0,
  ...
  const stats = [
    ...
    },
    {
      ...
      total: taskCounts['in progress'] || 0,
  ...
  return (
    <div classNamee='h-full py-4'>
      ...
      <div className='w-full flex flex-col md:flex-row gap-4 2xl:gap-10 py-8'>
        ...
        <TaskTable tasks={totalTask} />

        {/* /right */}

        <UserTable users={summary2} />
      </div>
    </div>
  );
};
